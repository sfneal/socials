...
docker logs -f socials

while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' socials) != true ]]; then
        break
    else
        echo "'socials' container is still running... waiting 3 secs then checking again..."
        sleep 3
    fi
done

# Confirm it exited with code 0
if [[ $(docker inspect -f '{{.State.ExitCode}}' socials) == 0 ]]; then
    echo "Success: Tests Passed! - stephenneal/socials:${TAG}"
else
    echo "Error: Tests Failed! - stephenneal/socials:${TAG}"
    exit 1
fi

# Confirm the image exists
docker image inspect stephenneal/socials:"${TAG}" > /dev/null 2>&1
